...
def get_args():
    parser = argparse.ArgumentParser(
        description="Utility script for uploading large asset files as a multipart upload " \
            "using the STAC API. When calling this script, summon can be used to inject secrets " \
                "from gopass into the environment variables. To do this, call this script like " \
                    "this: summon -p gopass -e dev|int|prod python3 multipart_upload_via_API.py " \
                        "localhost|DEV|INT|PROD collection-name item-name assset-name " \
                            "asset-file-path --part-size."
    )
    parser.add_argument(
        "env",
