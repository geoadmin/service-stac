...
class Collection(models.Model):
    ...
    id = models.TextField(primary_key=True,editable=False) # this is what is simply only called "id" in here: http://ltboc.infra.bgdi.ch/static/products/data.geo.admin.ch/apitransactional.html#operation/createCollection
    ...
    links = models.JSONField(blank=False) # containing an array of objects (Links (self, root and item), each with "href" and "rel"
    providers = models.JSONField() # containing an array of objects (providers, each with "name", "roles" and "url")
    ...
    summaries = models.JSONField() # this is, where some of the geocat-data could go. So this should contain "eo:gsd", "geoadmin:variant" and "proj:epsg"
...
class Items(models.Model):
    ...
    id = models.TextField(unique=True,blank=False)
    links = models.JSONField(blank=False) # containing an array of objects (Links (self, root and item), each with "href" and "rel"
    properties = models.JSONField(blank=False) # core metadata (e.g. datetime, eo:bands, eo:cloud_cover, eo:gsd, ...) fields plus extensions
    stac_extensions = models.JSONField()
    stac_version = models.TextField(blank=False)
    GeoJSON_type = models.TextField(default="Feature", blank=False, editable=False) # specifies the GeoJSON type and MUST be set to "Feature". Guess there is a more elegant way to prescribe a fixed value
    # other than giving a default value and don't allow editing.
    assets = models.JSONField() # this is defined as required here: https://github.com/radiantearth/stac-spec/blob/v0.9.0/item-spec/item-spec.md
...
class Assets(models.Model):
    ...
    geoadmin_lang = models.Textfield(
        max_length=2,
