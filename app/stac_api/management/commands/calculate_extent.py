...
class Handler(CommandHandler):
    ...
    def run(self):
        self.print_success('Runing command to update collection extents')
        # print(self.options)
        ...
        with connection.cursor() as cursor:
            for collection_id in collections:
                ...
                self.print_success(f"collection.id={collection_id} extent updated.")
        self.print_success(
            f"successfully updated extent of {len(collections)} collections in " +
            f"{(time.monotonic()-start):.3f}s"
        )


class Command(BaseCommand):
    help = """Calculate the collaction spacial and temporal extent for all collections that have
