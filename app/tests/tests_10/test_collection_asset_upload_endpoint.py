...
class CollectionAssetUploadBaseTest(StacBaseTestCase, S3TestMixin):
    ...
    def get_asset_upload_queryset(self):
        return CollectionAssetUpload.objects.all().filter(
            asset__collection__name=self.collection.name,
            asset__name=self.asset.name,
        )

    def get_delete_asset_path(self):
        return reverse_version(
            'collection-asset-detail', args=[self.collection.name, self.asset.name]
        )

    def get_get_multipart_uploads_path(self):
        return reverse_version(
            'collection-asset-uploads-list', args=[self.collection.name, self.asset.name]
        )

    def get_create_multipart_upload_path(self, collection=None, asset=None):
        return reverse_version(
            'collection-asset-uploads-list',
            args=[
    ...
    def get_abort_multipart_upload_path(self, upload_id, collection=None, asset=None):
        return reverse_version(
            'collection-asset-upload-abort',
            args=[
                collection.name if collection else self.collection.name,
                asset.name if asset else self.asset.name,
                upload_id
    ...
    def get_complete_multipart_upload_path(self, upload_id, collection=None, asset=None):
        return reverse_version(
            'collection-asset-upload-complete',
            args=[
                collection.name if collection else self.collection.name,
                asset.name if asset else self.asset.name,
                upload_id
    ...
    def get_list_parts_path(self, upload_id, collection=None, asset=None):
        return reverse_version(
            'collection-asset-upload-parts-list',
            args=[
                collection.name if collection else self.collection.name,
                asset.name if asset else self.asset.name,
                upload_id
    ...
    def s3_upload_parts(
        self, upload_id, file_like, size, number_parts, collection=None, asset=None
    ):
        s3 = get_s3_client()
        key = get_collection_asset_path(
            collection if collection else self.collection, asset.name if asset else self.asset.name
        )
        parts = []
        ...
        for part in range(1, number_parts + 1):
            ...
            response = s3.upload_part(
                ...
                Bucket=settings.AWS_SETTINGS['legacy']['S3_BUCKET_NAME'],
                Key=key,
                PartNumber=part,
                UploadId=upload_id
            )
...
class CollectionAssetUploadCreateEndpointTestCase(CollectionAssetUploadBaseTest):
    ...
    def test_asset_upload_create_abort_multipart(self):
        key = get_collection_asset_path(self.collection, self.asset.name)
        ...
        self.assertFalse(
            self.get_asset_upload_queryset().filter(
                status=CollectionAssetUpload.Status.IN_PROGRESS
            ).exists(),
            msg='In progress upload found'
        )
        self.assertTrue(
            self.get_asset_upload_queryset().filter(status=CollectionAssetUpload.Status.ABORTED
                                                   ).exists(),
        ...
        response = s3.list_multipart_uploads(
            Bucket=settings.AWS_SETTINGS['legacy']['S3_BUCKET_NAME'], KeyMarker=key
        )
...
class CollectionAssetUpload1PartEndpointTestCase(CollectionAssetUploadBaseTest):
    ...
    def upload_asset_with_dyn_cache(self, cache_control_header=None):
        ...
        asset_dyn_cache = self.factory.create_collection_asset_sample(
            collection=collection_dyn_cache, sample='asset-no-file'
        ).model

        key = get_collection_asset_path(collection_dyn_cache, asset_dyn_cache.name)
        self.assertS3ObjectNotExists(key)
        number_parts = 1
        size = 1 * KB
        ...
        response = self.client.post(
            self.get_create_multipart_upload_path(collection_dyn_cache, asset_dyn_cache),
            data={
                'number_parts': number_parts,
                'md5_parts': md5_parts,
                'file:checksum': checksum_multihash,
        ...
        parts = self.s3_upload_parts(
            ...
            size,
            number_parts,
            collection_dyn_cache,
            asset_dyn_cache
        )
        response = self.client.post(
            self.get_complete_multipart_upload_path(
                json_data['upload_id'], collection_dyn_cache, asset_dyn_cache
            ),
            data={'parts': parts},
            content_type="application/json"
        )
...
class CollectionAssetUploadDeleteInProgressEndpointTestCase(CollectionAssetUploadBaseTest):
    ...
    def test_delete_asset_upload_in_progress(self):
        ...
        self.assertEqual(
            response.json()['description'],
            ['Collection Asset collection-asset-1.tiff has still an upload in progress']
        )

        self.assertTrue(
            CollectionAsset.objects.all().filter(
                collection__name=self.collection.name, name=self.asset.name
            ).exists(),
        ...
        self.assertFalse(
            ...
            msg='Collection Asset has not been deleted'
        )
...
class GetCollectionAssetUploadsEndpointTestCase(CollectionAssetUploadBaseTest):
    ...
    def create_dummies_uploads(self):
        ...
        for i in range(1, 4):
            CollectionAssetUpload.objects.create(
                asset=self.asset,
    ...
    def test_get_asset_uploads_status_query(self):
        response = self.client.get(
            self.get_get_multipart_uploads_path(), {'status': CollectionAssetUpload.Status.ABORTED}
        )
        self.assertStatusCode(200, response)
        json_data = response.json()
        self.assertIn('uploads', json_data)
